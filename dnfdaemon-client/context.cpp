...
void RepoCB::start(sdbus::Signal & signal) {
    if (signature_valid(signal)) {
        ...
        progress_bar.set_state(libdnf::cli::progressbar::ProgressBarState::STARTED);
        progress_bar.start();
    }
}
...
void Context::init_session(Command * selected_command) {
    // open dnfdaemon-server session
    auto cfg = selected_command->session_config();
