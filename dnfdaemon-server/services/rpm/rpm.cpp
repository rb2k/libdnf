...
void packages_to_transaction(std::vector<DbusTransactionItem> & result, const libdnf::transaction::TransactionItemAction action, const std::vector<libdnf::rpm::Package> & packages) {
    for (auto & p: packages) {
        result.push_back(DbusTransactionItem(
...
void Rpm::resolve(sdbus::MethodCall && call) {
    auto worker = std::thread([this](sdbus::MethodCall call) {
        try {
            ...
            std::vector<DbusTransactionItem> result {};
